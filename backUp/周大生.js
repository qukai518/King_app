 ...
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.zds_data : $.getdata('zds_data')) || '';
 let msg, ck;
 let ck_status = true;
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "zds_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/member/center/signIn`,
			 headers: {
			  "host": host,
			  "Content-Length": "4",
		 ...
		 if (result?.code == 200) {
			 DoubleLog(`签到成功,签到获得:${result.data.original} 🎉积分`);
			 await wait(3);
  ...
  */
  async function check_value() {
	try {
		let url = {
			url: `${hostname}/member/point-shop/point-member/value`,
			headers: {
			 "host": host,
			 "Content-Length": "4",
			 "content-type": "application/json",
		...
		if (result?.code == 200) {
			DoubleLog(`查询成功,现有:${result.data} 🎉分`);
			await wait(3);
		} else {
